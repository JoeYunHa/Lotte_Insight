...
export function FanVoiceLane({ laneIndex, bubbles, onReact, onReport }: FanVoiceLaneProps) {
  return (
    <div className="fan-voice-lane" data-lane={laneIndex}>
      {bubbles.map((bubble) => (
        <FanVoiceBubble key={bubble.localId} bubble={bubble} onReact={onReact} onReport={onReport} />
      ))}
    </div>
  )
}
