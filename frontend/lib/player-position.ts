...
export const POSITION_GROUPS: { key: string; label: string; match: (p: string) => boolean }[] = [
  { key: 'pitcher', label: 'Pitchers', match: (p) => p.includes('투수') || p.toLowerCase().includes('pitcher') },
  { key: 'catcher', label: 'Catchers', match: (p) => p.includes('포수') || p.toLowerCase().includes('catcher') },
  {
    key: 'infield',
    label: 'Infielders',
    match: (p) =>
